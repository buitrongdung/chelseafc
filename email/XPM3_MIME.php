...
class XPM3_MIME extends XPM3_FUNC {
	...
	static public function unique($add = '') {
		return md5(microtime(1).$add);
	}

	static public function is_printable($str) {
		try {
	...
	static public function qp_encode($str, $len = self::MLEN, $end = self::CRLF) {
		try {
			$errors = array();
	...
	static public function encode_header($str, $charset = self::HCHARSET, $encoding = self::HENCODING) {
		try {
			$errors = array();
			if (!(is_string($str) && $str != '')) $errors[] = 'invalid argument type';
			if (!is_string($str)) $errors[] = 'invalid charset type';
			else {
	...
	static public function decode_header($str) {
		try {
			if (is_string($str)) {
				$out = '';
				$set = 'ISO-8859-1';
	...
	static public function decode_content($str, $decoding = '7bit') {
		try {
			$errors = array();
	...
	static public function message($content, $type, $name = '', $charset = self::MCHARSET, $encoding = self::MENCODING, $disposition = 'inline', $id = '', $exception = null) {
		try {
			$errors = array();
			if (!(is_string($content) && $content != '')) $errors[] = 'invalid content type';
			if (!is_string($type)) $errors[] = 'invalid type';
			else {
				$type = self::str_clear($type);
				$type = trim($type);
				$typelen = strlen($type);
				if ($typelen < 4 || $typelen > 22) $errors[] = 'invalid type value';
			}
			if (!is_string($name)) $errors[] = 'invalid name type';
				...
				if ($name != '') {
					$namelen = strlen($name);
					if ($namelen < 2 || $namelen > 22) $errors[] = 'invalid name value';
				}
			}
			...
			if (!is_string($disposition)) $errors[] = 'invalid disposition type';
			else {
			...
			if (!is_string($id)) $errors[] = 'invalid id type';
			else {
				$id = self::str_clear($id, array(' '));
				if ($id != '') {
					$idlen = strlen($id);
					if ($idlen < 2 || $idlen > 40) $errors[] = 'invalid id value';
				}
			}
			if (count($errors) == 0) {
	...
	static public function compose($text = false, $html = false, $attach = false, $uniq = null) {
		try {
			$errors = array();
			if ($text) {
			...
			if (!$text && !$html) $errors[] = 'message is not set';
			if ($attach) {
	...
	static public function split_message($body, $multipart, $boundary) {
		try {
			$errors = array();
	...
	static public function split_compose($str) {
		try {
	...
	static public function split_content($str) {
		try {
			if (!is_string($str)) throw new Exception('invalid argument type', 0);
			...
			if (isset($res['multipart'], $res['boundary'])) {
				...
				foreach ($res['header'] as $harr) {
					foreach ($harr as $hnum => $hval) if (stristr($hnm, 'content-')) $content[$hnum] = $hval;
				}
