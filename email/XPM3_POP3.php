...
class XPM3_POP3 extends XPM3_MIME {
	...
	static public function connect($host, $user, $pass, $port = self::PORT, $ssl = false, $timeout = self::TIMEOUT) {
		$fp = false;
		try {
			$errors = array();
			...
			if (is_string($ssl)) $ssl = strtolower(self::str_clear($ssl, array(' ')));
			if (!(is_bool($ssl) || (is_string($ssl) && isset(self::$_cssl[$ssl])))) $errors[] = 'invalid ssl value';
	...
	static public function pstat($conn) {
		try {
	...
	static public function plist($conn, $msg = 0) {
		try {
			$errors = array();
	...
	static public function pretr($conn, $msg) {
		try {
			$errors = array();
			if (!($conn && is_resource($conn))) $errors[] = 'invalid resource connection';
	...
	static public function pdele($conn, $msg) {
		try {
			$errors = array();
			if (!($conn && is_resource($conn))) $errors[] = 'invalid resource connection';
			if (!(is_int($msg) && $msg > 0)) $errors[] = 'invalid message number';
	...
	static public function pnoop($conn) {
		try {
			if (!($conn && is_resource($conn))) throw new Exception('invalid resource connection', 0);
	...
	static public function prset($conn) {
		try {
			if (!($conn && is_resource($conn))) throw new Exception('invalid resource connection', 0);
	...
	static public function pquit($conn) {
		try {
			if (!($conn && is_resource($conn))) throw new Exception('invalid resource connection', 0);
	...
	static public function receive($conn, $start = 0, $length = null) {
		try {
			$errors = array();
			if (!($conn && is_resource($conn))) $errors[] = 'invalid resource connection';
