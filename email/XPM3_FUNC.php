...
class XPM3_FUNC {
	...
	static public function is_alpha($strval, $numeric = true, $addstr = '') {
		try {
			$errors = array();
			if (!(is_string($strval) && $strval != '')) $errors[] = 'invalid value type';
	...
	static public function is_ipv4($str) {
		try {
			if (is_string($str) && $str != '') return (ip2long($str) && count(explode('.', $str)) === 4);
	...
	static public function is_mail($addr, $vermx = false) {
		try {
			...
			if (count($errors) == 0) {
				$ret = false;
				if (count($exp = explode('@', $addr)) === 2) {
					if ($exp[0] != '' && !(strstr($exp[0], '.-') || strstr($exp[0], '-.') || strstr($exp[0], '_-') || strstr($exp[0], '-_') || strstr($exp[0], '._') || strstr($exp[0], '_.')) && self::is_alpha($exp[0], true, '_-.') && (self::is_hostname($exp[1]) || self::is_ipv4($exp[1]))) {
						$ret = true;
						foreach (explode('-', $exp[0]) as $str1) {
							if ($str1 == '') {
								$ret = false;
								break;
							}
						...
						if ($ret) {
							foreach (explode('_', $exp[0]) as $str3) {
								if ($str3 == '') {
									$ret = false;
									break;
								}
