...
class XPM3_MAIL {
	...
	public function returnpath($addr = '') {
		try {
			if (!is_string($addr)) throw new Exception('invalid address type', 0);
			else {
				if ($addr == '') {
	...
	public function send($subject, $charset = XPM3_MIME::HCHARSET, $encoding = XPM3_MIME::HENCODING) {
		...
		try {
			...
			if (count($errors) == 0) {
				...
				$hxmail = array('name' => base64_decode('WC1NYWlsZXI='), 'value' => base64_decode('WFBNMyB2LjAuMSA8d3d3LnhwZXJ0bWFpbGVyLmNvbT4='));
				...
				foreach ($this->_vord as $delivery) {
					if (!$ret) {
						if ($delivery == 'relay') {
							...
							if (!mail(implode(', ', $ato), $subject, $replb, $replh)) {
